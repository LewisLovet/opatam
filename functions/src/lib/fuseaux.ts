/**
 * MIROIR de `packages/shared/src/utils/fuseaux.ts`.
 *
 * Ce paquet ne peut pas importer `@booking-app/shared` à l'exécution —
 * `firebase deploy` échoue dessus (source ESM). Le fichier d'origine a été
 * écrit sans la moindre dépendance EXPRÈS, pour être recopiable tel quel.
 *
 * ⚠ NE PAS MODIFIER ICI. Toute correction va dans la version partagée,
 * puis se recopie. Une divergence entre les deux redonnerait deux
 * interprétations du même calcul — c'est ce qui a coûté
 * `nextAvailableSlot` annonçant disponible un jour entièrement bloqué.
 *
 * ── Ce qui suit est la copie conforme ──────────────────────────────────
 */
