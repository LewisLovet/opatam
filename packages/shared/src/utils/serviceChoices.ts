/**
 * Helpers for building/sanitising service variations, options and info
 * fields. Pure logic (no React, no DOM) so it's shared by the web editor,
 * the registration wizard, and the mobile app.
 */
import type {
  ServiceInfoField,
  ServiceOption,
...
/** Stable client-side id for a freshly created choice. Uses crypto.randomUUID
 *  when available (web/Node), falls back to a random string (React Native). */
