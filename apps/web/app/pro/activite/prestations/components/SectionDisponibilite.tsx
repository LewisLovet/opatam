...
export function SectionDisponibilite({
  ...
  return (
    ...
    >
      ...
      {/* ── Jours de la semaine ─────────────────────────────────────────
          Restreindre une prestation à certains jours, en plus des horaires.
          Aucun jour coché = tous les jours : c'est l'état par défaut, et
          celui vers lequel « Tous les jours » ramène. */}
      <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between gap-4 mb-1">
          <label className="block text-sm font-medium text-gray-900 dark:text-white">
            Jours où cette prestation est proposée
          </label>
          {data.availableDays.length > 0 && (
            <button
              type="button"
              onClick={() => update({ availableDays: [] })}
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
          {data.availableDays.length === 0
            ? 'Proposée tous les jours où vous êtes ouvert.'
            : 'Vos horaires continuent de s’appliquer : un jour de fermeture reste fermé.'}
        </p>

        <div className="flex flex-wrap gap-2">
          {/* L'ordre commence au lundi — la semaine telle qu'on la lit —
              alors que les valeurs suivent Date.getDay() (0 = dimanche). */}
          {[
            [1, 'Lun'],
            [2, 'Mar'],
            [3, 'Mer'],
                ...
                onClick={() =>
                  update({
                    availableDays: selected
                      ...
                      : [...data.availableDays, day].sort((a, b) => a - b),
                  })
                }
