...
import { commissionOffreSurDouzeMois } from '@/lib/sales-offres';
...
interface OffreCatalogue {
  ...
  active: boolean;
  coupon?: { percentOff: number; duration: 'once' | 'repeating'; durationInMonths?: number };
}
...
export default function OffresPage() {
  ...
  const [tauxPct, setTauxPct] = useState<number | null>(null);
  ...
  const charger = async () => {
    ...
    if (moiRes.ok) {
      ...
      setRole(moi.role);
      setTauxPct(typeof moi.tauxCommissionPct === 'number' ? moi.tauxCommissionPct : null);
    }
  };
  useEffect(() => {
    void charger();
  ...
  // Invitation SANS remise — proposer une offre n'est jamais obligatoire.
  const envoyerInvitation = async () => {
    ...
    try {
      const res = await fetch('/api/sales/invitation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(await enTetesStaff()) },
        body: JSON.stringify({ email: email.trim(), message: messagePerso.trim() || undefined }),
      });
      const d = await res.json();
      if (!res.ok) {
        alert(d.error ?? 'Envoi impossible');
        return;
      }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
        ...
        <div className="p-4 grid sm:grid-cols-2 gap-3">
              ...
              {tauxPct !== null && o.coupon && (() => {
                ...
                return (
                  <p className="text-[11px] text-gray-400 mt-1.5">
                    ...
                    <span className="line-through tabular-nums">{euros(g.sansOffreCents)} €</span>
                    {' '}· l'offre vous coûte {euros(g.sansOffreCents - g.avecOffreCents)} €
                  </p>
                );
              })()}
              {o.active && (
          ...
          {/* Sans remise : le simple e-mail d'incitation, attribué */}
          <div className="rounded-xl border border-dashed border-gray-300 dark:border-gray-700 p-4">
            <p className="text-sm font-semibold text-gray-900 dark:text-white">
              Sans remise — simple invitation
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Un e-mail qui présente Opatam et l'essai gratuit, avec votre lien d'attribution.
              Aucune remise : votre commission reste entière.
            </p>
            <button
              onClick={() => {
                setOuverte(ouverte === 'invitation' ? null : 'invitation');
                setResultat(null);
                setInvitationEnvoyee(false);
              }}
              className="mt-2.5 inline-flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300 hover:underline"
            >
              <Mail className="w-3.5 h-3.5" /> Envoyer une invitation
            </button>
            {ouverte === 'invitation' && (
              <div className="mt-3 space-y-2 border-t border-gray-100 dark:border-gray-800 pt-3">
                <input
                ...
                <button
                  ...
                  disabled={generation || !email.trim()}
                  className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-xs font-semibold hover:opacity-90 disabled:opacity-50"
                >
                  {generation ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                  Envoyer l'invitation
                </button>
                {invitationEnvoyee && (
                  <p className="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 px-3 py-2.5 text-xs text-emerald-800 dark:text-emerald-300">
                    Invitation envoyée ✓ — l'inscription passera par votre lien d'attribution.
                  </p>
                )}
              </div>
            )}
