...
export function DemoEcran() {
  ...
  const [nbMembres, setNbMembres] = useState(3);
  ...
  return <div data-provider-theme>
    ...
    <aside className={s.panneau} data-ouvert={ouvert} aria-label="Options de démonstration">
      ...
      {ouvert && <div className={s.options}>
        ...
        <div className={s.groupe}><span>Couleur du salon</span><div className={s.couleurs}>
          {PROVIDER_THEMES.map((t) => <button type="button" key={t.id} title={t.label} aria-label={t.label} aria-pressed={themeId === t.id} onClick={() => setThemeId(t.id)} style={{ background: `rgb(${t.ramp[5]})` }} />)}
        </div></div>
        <label>Membres affichés <b>{nbMembres}</b><input type="range" min={1} max={5} value={nbMembres} onChange={(e) => setNbMembres(Number(e.target.value))} /></label>
